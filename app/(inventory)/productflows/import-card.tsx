...
export const ImportCard = ({ data, onCancel, onSubmit }: Props) => {
  ...
  const handleContinue = () => {
    ...
    const formattedData = arrayOfData.map((item) => ({
      label: item.label,
      minThroughput: Number(item.minThroughput),
      maxThroughput: Number(item.maxThroughput),
      sourceId: Number(item.sourceId),
      destinationId: Number(item.destinationId),
      vehicleTypeId: Number(item.vehicleTypeId),
      productId: Number(item.productId),
      timePeriodId: Number(item.timePeriodId),
